...
public class HotNinAdapter<T> extends BaseRecyclerViewAdapter<T> {

    public HotNinAdapter(List<T> list){
        updataData(list);
    }
    @Override
    ...
    public void displayViewHolder(BaseViewHolder holder, int position, T itemData) {
        DigitBean.ResultBean.HotProductListBean listBeans = (DigitBean.ResultBean.HotProductListBean) itemData;
